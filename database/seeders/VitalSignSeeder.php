        ...
        foreach ($residents as $resident) {
            ...
            foreach ($vitalSignsData as $vitalData) {
                VitalSign::create([
                    ...
                    'systolic_bp' => $vitalData['systolic'], // Fix column name
                    'diastolic_bp' => $vitalData['diastolic'], // Fix column name
                    'heart_rate' => $vitalData['pulse'], // Fix column name
                    'respiratory_rate' => rand(12, 20), // Add missing column
                    'oxygen_saturation' => $vitalData['oxygen_saturation'],
                    'weight' => rand(120, 200), // Add missing column
                    'height' => rand(60, 72), // Add missing column
                    'notes' => $vitalData['notes'],
                    'status' => $vitalData['status'],
                    'recorded_by' => $user->id, // Fix column name
                    'recorded_at' => Carbon::now()->subDays(rand(0, 7)), // Fix column name
                ]);
            }
        }
