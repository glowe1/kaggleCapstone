...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        // Create residents table or add missing columns if it exists
        if (!Schema::hasTable('residents')) {
            ...
            try {
                Schema::table('residents', function (Blueprint $table) {
                    $existingIndexes = collect(Schema::getConnection()->getDoctrineSchemaManager()->listTableIndexes('residents'))->keys()->toArray();
                    
                    if (!in_array('residents_branch_id_status_index', $existingIndexes)) {
                        $table->index(['branch_id', 'status'], 'residents_branch_id_status_index');
                    }
                    if (!in_array('residents_status_admission_date_index', $existingIndexes)) {
